...
//		사용자가 입력한 비밀번호를 암호화 시켜주고 비교하면 된다.
		String id = request.getParameter("user_id");
		String pw = request.getParameter("user_pw");
		
//		User u = new UserService().loginUser(id,pw);
		User u = new User(1,"useruser","PJkJr+wlNU1VHa4hWQuybjjVPyFzuNPcPu5MBH56scHri4UQPjvnumE7MbtcnDYhTcnxSkL9ei/bhIVrylxEwg==","김철수");
//		User u = null;
		//		1.User u = new UserService().loginUser(id,pw); 코드 주석처리
//		2.User 객체의 매개변수 생성자에 데이터 베이스 실제 값을 넣어서 User 구성
//		예시 ) new User(1,"admin","암호화된 비밀번호","관리자");
//		3.매개변수 생성자로 만든 객체 주석처리
//		4. User u = null; 로 실패 상황도 테스트 해보기
